...
from NISTgenerator.Port import CreateThroughPort, CreateThroughPortTilted, CreateThroughPortCurve
from NISTgenerator.Misc.label import CreateLabel

def CreateRingCurveWGShifted(fid, param, ncell):
                    ...
                    for r1 in R1:
                        for w in W:
                            ...
                            if cnt_shift>0:
                                ...
                                x_sleft = xpos1 - x_shift -rr
                                x_left = x_sleft - x_shift -rr
                                x_sleft_out = x_pos - x_shift -rr
                            ...
                            WG_through_port_y_pos = y_pos + (θ - Δ)/1.25
                            yc = y_pos -(r1-rr -g - w)
                            ...
                            if cnt_shift>0:
                                name_out += CreateThroughPortCurve(fid, params_port, ncell,cnt)
                            else:
                                print('ICI')
                            ...
                            cy2 = y_pos + ((θ - Δ))/1.25 #y_pos + (rr++w/2) - rr*rr/r1
                            x2 =  cx1+ 2*rr  
                            y2 = cy2
                            fid.write(str(layer) + ' layer\n')
                            ...
                            if len(LC)>1:
                                txt_LC = ' LC={:.3f}'.format(lc)
                            else:
                                txt_LC = ' '
                            if len(R1)>1:
                                txt_R1 = ' R1={:.3f}'.format(r1)
                            else:
                                txt_R1 = ' '

                            txt = txt_RR + txt_RW + txt_G + txt_W + txt_LC + txt_R1
                        
                            # txt = 'RR={:.3f}um RW={:.3f}um '.format(rr, rw) + \
                            #     'G={:.3f}um W={:.3f} Lc={:.0f}um R1={:.1f}'.format(g, w, lc, r1)
