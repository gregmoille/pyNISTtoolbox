...
def CreateWGRingModulateStraightWg(fid, param, ncell):
    ...
    for g in G:
        for rw in RW:
            for rr in RR:
                ...
                # Create the ring
                name_out.append(Name + 'RRCell' + str(ncell) + '_' + str(cnt))
                RWring = rr/3
                fid.write('<' + Name + 'RRCell' + str(ncell) +
                          '_' + str(cnt) + ' struct>\n')
                ...
                fid.write('\t{} {} '.format(x0, y_pos) +
                          '{} '.format(rr-rw) +
                          '{} {:.0f} '.format(rr-rw-exp_w, Nmodulation) +
                          '{} '.format(Amodulation) +
                          '10000 0 torusWaveIn\n')

                # x y ri r◦ n A Ns θ(x,y) torusWaveIn
