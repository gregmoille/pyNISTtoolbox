...
def GenericRing(Cells, param, Bloc):
    # ------------------------------------------
    ...
    for BB, yt in zip(Bloc, yTop):
        ...
        RR = {'name': f"R{ii_ring:02d}_",
                     'x0': 0,'y0': 0,'tot_length' : param['Wchip'],'exp_w': BB.get('exp_w', 2),
                     ...
                     'RWin' : BB.get('RW', None), 'RWetch': BB.get('RWetch', 0),
                     'G': BB.get('G', None),
                     ...
                     'y_pos_text': BB.get('y_pos_text', 0), 'x_pos_text': - param['Wchip']/2 + 400,
                     ...
                     'OAM': BB.get('OAM', False),
                     'symmetrize': BB.get('symmetrize', False),
                     'ccw_drop': BB.get('ccw_drop', True),
                     'output_inv_taper_W_drop': BB.get('WtapperDrop', 0.15),
                     'neff_fun': BB.get('neff_fun', None),'neff0': BB.get('neff0', None),
                     'ThreeD': BB.get('ThreeD', False),
                     }
        ...
        Ntot = len(BB.get('G', [0])) *  len(BB.get('RW', [0])) * len(BB.get('Lc', [0])) * \
              ...
              len(BB.get('Gdrop', [0])) * len(BB.get('LcDrop', [0]))
        ...
        Fbox = {'Name':f'R{ii_ring:02d}_Fld','Hbox': Hbox ,
                'Nx':1,
                'ypitch': BB.get('ypitch', Hbox),
                'increment': increment, 
                'Wbox': param['Wbox'], 'y0': -2, 'layer': param['layerField'],
                'yshift0': 4.9,#-Bloc[0]['ypitch']/2,
                'N': Ntot, 'Wchip': param['Wchip']}
        ...
        GdropLabell = {'name': f'R{ii_ring:02d}_Lbl', 'layer': param['layer_label'],
                     'box': True, 'font': 'Source Code Pro',
                     'font_size_pattern': 20,'txt': BB['Label'],
                     'x_pos_text': -350, 'y_pos_text': + 28,
                     }
