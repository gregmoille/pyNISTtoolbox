...
def CreateThroughDropPortSeparated(fid, param, ncell, cnt_out):
    ...
    if polarity == 'positive':
        fid.write('\t<{:.3f} {:.3f} '.format(x1_out_wg+150, y_drop+ydropshift) +
                  '{:.3f} {:.3f} '.format(x1_out_wg+155, y_drop+ydropshift) +
    ...
    if polarity == 'positive':
        fid.write('\t<{:.3f} {:.3f} '.format(x1_out_wg+150, y_drop+ydropshift) +
                  '{:.3f} {:.3f} '.format(x1_out_wg+100, y_drop+ydropshift) +
    ...
    if polarity == 'positive':
        fid.write('\t<{:.3f} {:.3f} '.format(x1_out_wg, y_drop) +
                  '{:.3f} {:.3f} '.format(100, ydropshift) +
    ...
    if polarity == 'positive':
        fid.write('\t<{:.3f} {:.3f} '.format(x2_in_wg, y_drop) +
                  '{:.3f} {:.3f} '.format(-100, ydropshift) +
                  '{:.3f} {:.3f} '.format(Wdrop, exp_w_tapper) +
                  '0 sBendInv>\n')
    else:
    ...
    # fid.write('\t<{:.3f} {:.3f} '.format(x2_in_wg-105, y_drop+ydropshift) +
    #           '{:.3f} {:.3f} '.format(x2_in_wg -100, y_drop+ydropshift) +
    #           '{:.3f} {:.3f} '.format(W, exp_w) +
    #           '0 0 0 {}>\n'.format(wvg_type))
    W2_in_lin = Wdrop+2*exp_w
    if polarity == 'positive':
        fid.write('\t<{:.3f} {:.3f} '.format(x2_in_wg-100, y_drop+ydropshift) +
                  '{:.3f} {:.3f} '.format(x2_in_wg -205, y_drop+ydropshift) +
        ...
        fid.write('\t<{:.3f} {:.3f} '.format(x2_in_wg-100, y_drop+ydropshift) +
                  ...
                  '0 linearTaper>\n')
    if polarity == 'positive':
        fid.write('\t<{} {} '.format(x2_in_wg -205, y_drop+ydropshift) +
                  '{} {} '.format(-RR,-RR) +
                  '{} {} '.format(Wbend, exp_w) +
                  '0 90degreeBendInv>\n')
    else:
        fid.write('\t<{} {} '.format(x2_in_wg -205, y_drop+ydropshift) +
                  ...
                  '0 90degreeBendLH>\n')
    if polarity == 'positive':
        fid.write('\t<{} {} '.format(x2_in_wg -205-RR,y_drop+ydropshift-RR) +
                  '{} {} '.format(RR,RR) +
                  '{} {} '.format(Wbend, exp_w) +
                  '-90 90degreeBendInv>\n')
    else:
    ...
    yout = y_drop+ydropshift-2*RR


    # fid.write('\t<{:.3f} {:.3f} '.format(x2_in_wg-100, y_drop+ydropshift) +
    #           '{:.3f} {:.3f} '.format(x2_in_wg-100 - 3*RR, y_drop + ydropshift) +
    #           '{:.3f} {:.3f} '.format(x2_in_wg-100 - 3*RR , y_drop +RR-ydropshift) +
    #           '{:.3f} {:.3f} '.format(x2_in_wg-100, y_drop+RR-ydropshift) +
    #           '{:.3f} {:.3f} '.format(exp_w, W) +
    #           '0 bezierCurveInv>\n')
    if polarity == 'positive':
        fid.write('\t<{:.3f} {:.3f} '.format(x2_in_wg -205, yout) +
                  '{:.3f} {:.3f} '.format(x2_in_wg-100, yout) +
        ...
        fid.write('\t<{:.3f} {:.3f} '.format(x2_in_wg -205, yout) +
                  ...
                  '0 linearTaper>\n')
    if polarity == 'positive':
        fid.write('\t<{:.3f} {:.3f} '.format(x2_in_wg-100, yout) +
                  '{:.3f} {:.3f} '.format(x1_out_wg, yout) +
    ...
    We_in_wg = exp_w
    if polarity == 'positive':
        fid.write('\t<{} {} '.format(x1_out_wg, yout) +
